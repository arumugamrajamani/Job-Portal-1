<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($currentpassword)) {
            $currentpasswordRR = array('status' => 'error', 'message' => 'CurrentPassword is required.');
        } elseif(strlen($currentpassword) < 8 || strlen($currentpassword) > 30) {
            $currentpasswordRR = array('status' => 'error', 'message' => 'CurrentPassword must be between 8 and 30 characters.');
        } else {
        ...
        if($passwordRR['status'] == 'success' && $confirmpasswordRR['status'] == 'success') {
            ...
            //if its an admin it will look for the email and otp code in the admin table
                //change the password of the email in the database
